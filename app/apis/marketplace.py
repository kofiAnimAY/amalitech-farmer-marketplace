...
class ListingResource(Resource):
    ...
    def post(self):
        ...
        listing_id = marketplace.add_listing(item, listed_by, price, quantity, description)
    ...
    def patch(self):
        ...
        if new_price is not None:
            result = marketplace.update_price(listing_id, new_price)
            if result["message"] != "Price updated successfully":
                return {"message": result["message"]}, HTTPStatus.NOT_FOUND
        
        if new_name is not None:
            result = marketplace.update_name(listing_id, new_name)
            if result["message"] != "Name updated successfully":
                return {"message": result["message"]}, HTTPStatus.NOT_FOUND

        if description is not None:
            result = marketplace.update_description(listing_id, description)
            if result["message"] != "Description updated successfully":
                return {"message": result["message"]}, HTTPStatus.NOT_FOUND
        
        if new_quantity is not None:
            result = marketplace.set_quantity(listing_id, new_quantity)
    ...
    def delete(self):
        ...
        result = marketplace.delete_listing(listing_id)
